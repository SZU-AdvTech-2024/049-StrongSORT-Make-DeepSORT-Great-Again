...
class opts:
    def __init__(self):
        ...
        self.parser.add_argument(
            '--path_AFLink',
            default='StrongSORT/AFLink_epoch20.pth'
        )
        self.parser.add_argument(
            '--dir_save',
            default='output_strongsort++'
        )
        self.parser.add_argument(
            '--EMA_alpha',
            default=0.9
    ...
    def parse(self, args=''):
        ...
        if opt.BoT:
            ...
            opt.dir_dets = 'StrongSORT/{}_{}_YOLOX+BoT'.format(opt.dataset, opt.mode)
            # opt.dir_dets = 'features_MOT16_vitg'
        else:
            opt.max_cosine_distance = 0.3
            opt.dir_dets = 'StrongSORT/{}_{}_YOLOX+simpleCNN'.format(opt.dataset, opt.mode)
        if opt.MC:
        ...
        if opt.EMA:
            ...
            opt.nn_budget = 100
        if opt.ECC:
            path_ECC = 'StrongSORT/{}_ECC_{}.json'.format(opt.dataset, opt.mode)
